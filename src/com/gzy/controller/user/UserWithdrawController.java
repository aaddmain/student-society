...
public class UserWithdrawController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            ...
            // 检查余额是否充足
            if (userAmount.getBalance() < amount) {
                System.out.println("余额不足。用户余额: " + userAmount.getBalance() + ", 请求金额: " + amount);
                ResponseUtil.fail(resp, ErrorCode.INSUFFICIENT_BALANCE);
                Database.rollbackTransaction();
                return;
            }
            ...
            if (result > 0) {
                ...
                resultObj.put("message", "取款成功");
                resultObj.put("newBalance", newBalance / 100.0); // 转换为元
                resultObj.put("withdrawnAmount", amount / 100.0); // 转换为元
                ResponseUtil.success(resp, resultObj);
                System.out.println("取款成功，返回结果");
            } else {
