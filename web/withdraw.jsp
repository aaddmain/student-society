...
<body>
    ...
    <div class="container">
        <div class="card">
            <h2><i class="fas fa-arrow-up"></i> 取款</h2>
            <form id="withdrawForm">
                ...
                <div class="form-group">
                    <label class="form-label" for="amount">取款金额 (元)</label>
                    <input type="number" id="amount" class="form-input" placeholder="请输入取款金额" step="0.01" min="0.01">
                </div>
    ...
    <script>
        ...
        async function handleWithdraw(e) {
            ...
            // 显示加载状态
            const submitBtn = document.querySelector('#withdrawForm .btn');
            ...
            try {
                ...
                const response = await fetch('<%= request.getContextPath() %>/user/withdraw', {
                    method: 'POST',
                    headers: {
                ...
                if (result.success) {
                    ...
                    let message = '取款成功！';
                    if (data.newBalance !== undefined && data.withdrawnAmount !== undefined) {
                        message = '取款成功！取款金额：¥' + parseFloat(data.withdrawnAmount).toFixed(2) + '，当前余额：¥' + parseFloat(data.newBalance).toFixed(2);
                    } else if (data.newBalance !== undefined) {
                        message = '取款成功！当前余额：¥' + parseFloat(data.newBalance).toFixed(2);
                    }
                    alert(message);
                    // 清空表单
                    ...
                    switch(result.message) {
                        ...
                        case '用户密码错误':
                            ...
                            break;
                        case '账户余额不足':
                            errorMessage = '账户余额不足，无法完成取款';
                            break;
                        case '系统错误':
                            errorMessage = '系统发生错误，请稍后重试';
                            break;
                    }
                    alert('取款失败：' + errorMessage);
                }
            } catch (error) {
